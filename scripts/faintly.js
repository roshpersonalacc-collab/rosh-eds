...
async function resolveTemplate(context) {
  ...
  context.template.path =
    context.template.path ||
    `${context.codeBasePath}/blocks/${context.blockName}/${context.blockName}.html`;
  const templateId = `faintly-template-${context.template.path}#${context.template.name || ''}`
    .toLowerCase()
    .replace(/[^0-9a-z]/gi, '-');
  ...
  if (!template) {
    ...
    if (!resp.ok)
      throw new Error(
        `Failed to fetch template from ${context.template.path} for block ${context.blockName}.`,
      );
    ...
    templateDom.querySelectorAll('template').forEach((t) => {
      ...
      t.id = `faintly-template-${context.template.path}#${name}`
        .toLowerCase()
        .replace(/[^0-9a-z]/gi, '-');
...
async function processTest(el, context) {
  const testAttrName = el
    .getAttributeNames()
    .find(
      (attrName) => attrName.startsWith('data-fly-test') || attrName.startsWith('data-fly-not'),
    );
...
async function processRepeat(el, context) {
  const repeatAttrName = el
    .getAttributeNames()
    .find((attrName) => attrName.startsWith('data-fly-repeat'));
  ...
  const repeatedNodes = await Promise.all(
    Object.entries(arr).map(async ([key, item], i) => {
...
async function processNode(node, context) {
  ...
  if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    processChildren =
      (await processContent(node, context)) || (await processInclude(node, context)) || true;
...
export { renderBlock, renderElement };
